...
public class Money implements Comparable, Serializable {
	...
	public static Money cents(long centAmount) {
		return Money.cents(new BigDecimal(centAmount));
	}

	public static Money cents(BigDecimal centAmount) {
		if(centAmount.scale() > 0)
			throw new IllegalArgumentException("invalid cent format");

		BigDecimal dollarAmount = centAmount.divide(new BigDecimal(100));
		return Money.valueOf(dollarAmount, USD);
	}

	public static Money yens(BigDecimal amount) {
		return Money.valueOf(amount, JPY);
	}
	public static Money yens(long amount) {
		return Money.valueOf(amount, JPY);
	}

	public static Money wons(BigDecimal amount) {
		return Money.valueOf(amount, KRW);
	}
	...
	/**
	 * How best to handle access to the internals? It is needed for database
	 * mapping, UI presentation, and perhaps a few other uses. Yet giving public
	 * access invites people to do the real work of the Money object elsewhere.
	 * Here is an experimental approach, giving access with a warning label of
	 * sorts. Let us know how you like it.
	 */
	...
	// TODO: Provide some currency-dependent formatting. Java 1.4 Currency
	// doesn't
	// do it.
